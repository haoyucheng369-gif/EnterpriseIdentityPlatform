...
namespace EnterpriseIdentityPlatform.ApiServer.Tests;
...
public sealed class ContentEndpointTests : IClassFixture<ApiServerFactory>
{
    ...
    {
        // 杩欎釜 token 浣跨敤 Entra issuer/audience 鍜岀嫭绔嬬鍚?key锛屽彧閰嶇疆缁?EntraJwt scheme銆?
        // 濡傛灉璇锋眰鎴愬姛锛岃鏄?SmartBearer 姝ｇ‘鍒嗘祦鍒颁簡 EntraJwt锛岃€屼笉鏄璧?LocalJwt銆?
...
public sealed class ApiServerFactory : WebApplicationFactory<Program>
{
    ...
    public string CreateEntraToken(string subject, string scp)
    {
        ...
        // 杩欓噷妯℃嫙 Entra ID access token 鐨勫叧閿瓧娈碉紝閲嶇偣瑕嗙洊 issuer銆乤udience 鍜?scp銆?
        var token = new JwtSecurityToken(
    ...
    {
        builder.UseContentRoot(FindProjectDirectory("EnterpriseIdentityPlatform.ApiServer"));
        ...
        builder.ConfigureServices(services =>
        {
            ...
            services.PostConfigure<JwtBearerOptions>("EntraJwt", options =>
            {
                ...
                // 娴嬭瘯鐜涓嶈闂?Microsoft discovery endpoint锛岀洿鎺ユ敞鍏ユā鎷熺殑 Entra JWKS 閰嶇疆銆?
                options.Configuration = new OpenIdConnectConfiguration
                {
                    Issuer = "https://login.microsoftonline.com/976c3c85-e425-4880-a658-3653df9cebf2/v2.0"
                };
